...
当前版本：`3.3.1`
开发中（未发布）：远程识屏改为默认按需截图，识屏入口请求新帧并落实强制重拍，图片与窗口信息整帧原子提交；「只截取活动窗口」在远程模式下由客户端裁剪；远程模式的系统状态与活动窗口改读客户端上报数据，本机专属能力（输入统计、麦克风触发、浏览器历史）明确不可用而不静默顶替。
...
远程模式下插件运行在服务器上，用户桌面在另一台机器。凡是与“用户设备”有关的数据都必须来自客户端，插件不会用服务器本机数据顶替。

| 数据 | 远程来源 | 说明 |
| --- | --- | --- |
| 屏幕画面 | 客户端随帧上报 | 按需采集，见上文 |
| 活动窗口标题 | 同一帧的 `window_title` | 与图片一起原子提交，不会错位 |
| 窗口裁剪 | 客户端执行 | 服务端只记录 `capture_scope`，不再拿矩形裁剪 |
| 系统负载与电量 | 同一帧的 `system_stats` | 仅 `--push` 周期推送会采样；按需路径默认不带，此时不产生系统状态提示 |
| 录屏 | 客户端 `--video` 周期上传 | 服务端只读最近一次已上传的短片 |

**远程模式明确不可用的能力**（不会静默改用服务器数据）：

- **本地输入统计**（键盘节奏、在场感、离开自动挂起）：客户端尚未上报输入统计，开启后状态会显示为不可用，插件不会去监听服务器的键鼠。
- **麦克风音量触发识屏**：客户端尚未上报音量，远程模式下不会启动服务器麦克风监听。请改用自动观察或 `/kp`。
- **浏览器历史旁证**：读取的是服务器浏览器数据库，与用户无关，因此远程模式下不会返回任何候选。

**活动轨迹的时间分辨率低于本机模式**：远程窗口标题只在发生识屏时更新，空闲期间不会刷新。插件会用 `remote_screenshot_max_age` 判断最近一帧是否过期，过期帧不参与活动计时，避免一次识屏被记成持续数小时的同一窗口。健康检查里会显示当前帧是否有效。

`/kpr` 在远程模式下分析的是客户端**最近一次已上传**的录屏，而不是现拍画面：远程协议没有“命令客户端立刻补录一段”的能力，因此提示文案会明确说明，不会说成“正在录制”。需要更实时的画面请用 `/kp`。
